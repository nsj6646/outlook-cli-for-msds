...
* **다중 참조(Cc) 및 비밀참조(Bcc) 바인딩**: 엑셀 명단 내 세미콜론 `;` 구분자를 사용해 참조(Cc) 및 비밀참조(Bcc) 수신인을 다중 지정할 수 있습니다.
* **제목/본문 수신인 정보 자동 가공**: 메일 제목에 수신처 회사명(Company)이 자동 덧붙여지며, 본문 최상단에 `"수신: [고객사명] [담당자명]님"` 격식 있는 머리글을 강제로 자동 조립해 냅니다. (정보 누락 시 해당 수신인은 스킵됩니다.)
* **실패(스킵) 건 재송용 엑셀 자동 백업**: 누락 등으로 스킵 처리가 단 1건이라도 발생하면 완료 시점에 현재 디렉토리에 `[원본파일명]_skipped_[YYYYMMDD_HHMMSS].xlsx` 파일을 자동 백업합니다. 오류 수정 후 이 파일 그대로 재구동하여 부분 재송할 수 있습니다.
...
| To | Cc | Bcc | Subject | Body | Company | Contact | From | 예약시간 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 수신자 이메일 | 참조자 이메일 (여러 명일 경우 `;` 구분) | 비밀참조자 이메일 (여러 명일 경우 `;` 구분) | 메일 제목 | 메일 본문 내용 (`{{table}}` 플레이스홀더 포함 가능) | 고객사명 (필수) | 담당자명 (필수) | 아웃룩 등록 발신계정 주소 | 예약 발송 일시 (포맷: `YYYY-MM-DD HH:MM:SS`) |
