...
| To | Cc | Subject | Body | From | 예약시간 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 수신자 이메일 | 참조자 이메일 (여러 명일 경우 `;` 구분) | 메일 제목 | 메일 본문 내용 (`{{table}}` 플레이스홀더 포함 가능) | 아웃룩 등록 발신계정 주소 | 예약 발송 일시 (포맷: `YYYY-MM-DD HH:MM:SS`) |

### (2) `TableData` 시트 (제품 취급 정보)
| Email | 번호 | 제품코드 | 제품명 |
| :--- | :--- | :--- | :--- |
| 수신인 이메일 (수신처와 매핑 키) | 일련번호 | 제품 코드 (예: `P1003`) | 취급 제품 명칭 (예: `알파`) |
