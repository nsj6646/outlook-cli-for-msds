...
| To | Cc | Bcc | Subject | Company | Contact | Body | From | 예약시간 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 수신자 이메일 | 참조자 이메일 (여러 명일 경우 `;` 구분) | 비밀참조자 이메일 (여러 명일 경우 `;` 구분) | 메일 제목 | 고객사명 (필수) | 담당자명 (필수) | 메일 본문 내용 (`{{table}}` 플레이스홀더 포함 가능) | 아웃룩 등록 발신계정 주소 | 예약 발송 일시 (포맷: `YYYY-MM-DD HH:MM:SS`) |
